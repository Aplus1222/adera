...
row_func_pattern = r'fun VaultItemRow\([\s\S]*?fun QuickAddSocialMedia'

def modify_row(match):
    row_code = match.group(0)
    
    # 1. Add `passwordVisible` state
    row_code = re.sub(
        r'val haptic = LocalHapticFeedback\.current',
        'val haptic = LocalHapticFeedback.current\n    var passwordVisible by remember { mutableStateOf(false) }',
        row_code
    )
    
    # 2. Add brand icon
    # Search for Column(modifier = Modifier.weight(1f))
    ...
    # Replace the Row containing the password
    pass_row_pattern = r'Row\(\s*verticalAlignment = Alignment\.CenterVertically,\s*horizontalArrangement = Arrangement\.SpaceBetween,\s*modifier = Modifier\.fillMaxWidth\(\)\s*\)\s*\{\s*Text\(\s*text = "•••• ••••",[\s\S]*?\}\s*\}\s*\}'
...
new_content = re.sub(row_func_pattern, modify_row, content)
